...
# --- PAGE: QUESTIONNAIRE ---
    ...
    with st.form("form_survey"):
        ...
        categories = [
            {"key": "Mindset", "title": "Bagian I: Mindset Berorientasi Pelayanan"},
            {"key": "Culture", "title": "Bagian II: Culture-Set Berorientasi Pelayanan"}
        ]
        for cat_info in categories:
            st.subheader(cat_info["title"])
            cat_questions = [q for q in questions if q.get("category") == cat_info["key"]]
            
            for idx, q in enumerate(cat_questions, 1):
                st.write(f"**{idx}. {q['scenario']}**")
                # Pre-select answer if already answered
                current_val = st.session_state.answers.get(str(q["id"]), {}).get("text", None)
                idx_val = likert_options.index(current_val) if current_val in likert_options else None
